...
if app_mode == "ナレッジベース管理":
    ...
    with tab2:
        ...
        if kb_list_for_mng:
            ...
            if selected_kb_to_manage:
                ...
                with col_mng2:
                    if st.button(f"🗑️ 「{selected_kb_to_manage}」を削除", type="secondary", key=f"delete_kb_btn_{selected_kb_to_manage}", use_container_width=True, help="この操作は元に戻せません！"):
    ...
    with st.expander("ℹ️ RAGシステムのベストプラクティスとヒント"): 
        st.markdown("""
...
"""
    ...
    # --- ユーザー入力 ---
    ### MODIFIED ### - st.chat_input の key を動的に変更
    ...
    if user_chat_input_val:
        ...
        with st.chat_message("assistant"):
            placeholder = st.empty() ### ADDED ### - プレースホルダーで応答待ち表示
            placeholder.markdown("🤔 考え中...") ### ADDED ###
            ...
            gpt_response_text_val = "" ### ADDED ### - 応答テキスト初期化
            if not client_for_chat_resp:
                gpt_response_text_val = "エラー: OpenAIクライアントに接続できません。"
                placeholder.error(gpt_response_text_val) ### MODIFIED ###
            else:
                try: ### ADDED ### - エラーハンドリング
                    ...
                    placeholder.markdown(gpt_response_text_val) ### MODIFIED ###
                except Exception as e: ### ADDED ### - エラーハンドリング
                    logger.error(f"GPT応答生成中にエラー: {e}", exc_info=True)
                    gpt_response_text_val = f"申し訳ありません、応答の生成中にエラーが発生しました。"
                    placeholder.error(gpt_response_text_val) ### ADDED ###
            
            # 3. アシスタントの応答をセッションに追加
            if gpt_response_text_val: ### MODIFIED ### - 空でない場合のみ、またはエラーメッセージも追加
    ...
    # --- 元のアシスタント応答処理ブロックは削除（上記に統合されたため） ---
    # if st.session_state.get('gpt_messages') and \
    #    st.session_state.gpt_messages[-1]["role"] == "user" and \
    #    len(st.session_state.gpt_messages) > 0:
    #       # このブロックの内容は上に移動しました

    # --- サイドバーの操作ボタン ---
    if st.sidebar.button("🗑️ 新しい会話を開始する", key="new_chat_button_main", type="secondary", use_container_width=True):
        st.session_state.gpt_messages = []
        st.session_state.gpt_conversation_id = str(uuid.uuid4())
        st.session_state.gpt_conversation_title = "新しい会話"
        st.rerun() 
...
st.markdown("---")
st.markdown("<div style='text-align: center;'>RAGシステム統合ツール v1.1.0 - AIによる知識活用を支援</div>", unsafe_allow_html=True)
