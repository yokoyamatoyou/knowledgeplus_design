...
# メインタブ
tab1, tab2, tab3 = st.tabs(["📤 画像アップロード", "✏️ 内容編集・ナレッジ化", "📊 ナレッジベース管理"])
        ...
        if metadata_files:
            ...
            if kb_metadata_path.exists():
                try:
                    ...
                    with col_stat2:
                        st.metric("🖼️ 画像", kb_info.get('item_types', {}).get('image', len(metadata_files)))
                    with col_stat3:
...
├── 🖼️ images/      ({len(list(images_dir.glob('*.*')) if images_dir.exists() else [])}件)
...
st.markdown(
    ...
    "マルチモーダルナレッジ構築ツール v3.0.0 - 既存RAGシステム統合対応版"
    "</div>", 
